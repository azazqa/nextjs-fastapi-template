...
백엔드 `get_current_user`는 Redis SET(`user_perms:{uuid}`, `user_roles:{uuid}`)에 TTL 300초로 권한을 캐시한다. PG가 SSOT이며, `grant-role`·`seed-rbac` CLI가 캐시를 무효화한다. `REDIS_URL` 미설정 시 매 요청 PG 조회.

### 이후 최적화

다른 경로(예: 프론트 DAL)에서 별도 캐시를 두면 동일하게 **권한 변경 시 무효화** 경로가 필요하다.
